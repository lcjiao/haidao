package com.islandback.action.company;
...
@Namespace("/company/address")
@ResultPath("/WEB-INF")
public class CompanyaddressAction extends ActionSupport {
	...
	public String add(){
		...
		recommend.setModuleId(ModuleEnum.COMPANY_ADDRESS);
		if(image != null ){
	...
	private void doList(){
		...
		Map<String,Object> params = new HashMap<String,Object>(0);
		params.put("moduleId", ModuleEnum.COMPANY_ADDRESS);
		params.put("valid", 1);
		Page page = new Page();
		...
		if(list != null && list.size()>0){
			...
			countParam.put("moduleId", ModuleEnum.COMPANY_ADDRESS);
