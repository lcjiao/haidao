					...
					<tbody id="r_list">
						<c:forEach var="moduleType" items="${moduleTypeList}">
							<tr >
								...
								<td style="text-align:center;">
									<c:choose>
									   <c:when test="${moduleType.packageType==130}">
									   		摄影案例
									   </c:when>									   
									   <c:otherwise>
											视频案例
...
<script>
	...
	function newCreate(){
		var url = "${ctx}/customercase/moduletype/areamoduletype!toAdd.action";
		window.location.href = url;
	};
	...
	function del(ele){
		...
		if(isHide){
			...
			var url = "${ctx}/customercase/moduletype/areamoduletype!del.action?id="+recommendId;
			window.location.href = url; 
		}
	}
	...
	function edit(ele){
		var recommendId = $(ele).attr('title');
		var url =  "${ctx}/customercase/moduletype/areamoduletype!toEdit.action?id="+recommendId;
		window.location.href = url; 
	}
