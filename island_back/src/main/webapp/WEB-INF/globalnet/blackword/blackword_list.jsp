		...
		<tr>
			<td colspan="2">
				...
				<table class="datalist ask_rel" width="100%">
					...
					<tbody id="r_list">
						<c:forEach var="word" items="${blackwordList}">
							<tr >
								<td style="text-align:center;">
									<c:out value="${word.name}"></c:out>
								</td>
								<td style="text-align:center;">
									<a title="${word.id}" onclick="edit(this)" >修改</a>&nbsp;|&nbsp;
									<a title="${word.id}" onclick="del(this)">删除</a>
								</td>							
						    </tr>
					    </c:forEach>
					</tbody>
...
<script>
	...
	function newCreate(){
		var url = "${ctx}/globalnet/blackword/blackword!toAdd.action";
		window.location.href = url;
	};
	...
	function del(ele){
		...
		if(isHide){
			...
			var url = "${ctx}/globalnet/blackword/blackword!del.action?id="+id;
			window.location.href = url; 
		}
	}
	...
	function edit(ele){
		var id = $(ele).attr('title');
		var url =  "${ctx}/globalnet/blackword/blackword!toEdit.action?id="+id;
		window.location.href = url; 
	}
