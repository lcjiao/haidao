						...
						<c:forEach var="qaType" items="${globalQaTypeList}">
							<tr >
								<td style="text-align:center;">
									<c:out value="${qaType.typeName}"></c:out>
								</td>
								<td style="text-align:center;">
									<a title="${qaType.id}" onclick="edit(this)" >修改</a>&nbsp;|&nbsp;
									<a title="${qaType.id}" onclick="del(this)">删除</a>
								</td>							
						    </tr>
					    </c:forEach>
					</tbody>
...
<script>
	...
	function findByNo(pageNo){
		var url = "${ctx}/globalqa/globalqatype/globalqatype!tolist.action?pageNo="+pageNo;
		window.location.href = url;
	}
	
	
	function newCreate(){
		var url = "${ctx}/globalqa/globalqatype/globalqatype!toAdd.action";
		window.location.href = url;
	};
	
	function del(ele){
		var isHide = confirm('删除类型将会同步删除该类型下问题,确定删除吗?');
		if(isHide){
			var recommendId = $(ele).attr('title');
			var url = "${ctx}/globalqa/globalqatype/globalqatype!del.action?id="+recommendId;
			window.location.href = url; 
		}
	}
	...
	function edit(ele){
		var recommendId = $(ele).attr('title');
		var url =  "${ctx}/globalqa/globalqatype/globalqatype!toEdit.action?id="+recommendId;
		window.location.href = url; 
	}
