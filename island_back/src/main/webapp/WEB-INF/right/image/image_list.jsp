...
<form action="${ctx}/right/image/image!list.action" id="form" method="post">
<table class="searchbar" width="100%">
	<tbody>
		<tr>
			<td width="48">图片描述</td>
			<td width="48">
				<input type="text" name="img.imgDesc" id="img_desc" value=""/>
			</td>
			<td width="48">
				每页显示数：
			</td>
						...
						<c:forEach var="img" items="${imgList}">
							<tr >
								<td style="text-align:center;">
									<c:out value="${img.imgUrl}"></c:out>
								</td>
								<td style="text-align:center;">
									<c:out value="${img.imgDesc}"></c:out>
								</td>
								<td style="text-align:center;">
									<img style="width:150px;height:120px;" alt="" src="${img.imgUrl}">
								</td>
								<td style="text-align:center;">
									<a title="${img.id}" onclick="edit(this)" >修改</a>&nbsp;|&nbsp;
									<a title="${img.id}" onclick="del(this)">删除</a>
								</td>							
						    </tr>
					    </c:forEach>
					</tbody>
...
<script>
	...
	function newCreate(){
		var url = "${ctx}/right/image/image!toAdd.action";
	...
	function del(ele){
		...
		if(isHide){
			...
			var url = "${ctx}/right/image/image!del.action?id="+imgId;
			window.location.href = url; 
		}
	}
	...
	function edit(ele){
		var imgId = $(ele).attr('title');
		var url =  "${ctx}/right/image/image!toEdit.action?id="+imgId;
		window.location.href = url; 
	}
